...
func TestPlatformUpdateWaitOutcomeClassification(t *testing.T){signaledErr:=exec.Command("sh","-c","kill -KILL $$").Run();if signaledErr==nil||!platformUpdateWaitOutcomeUnknown(signaledErr){t.Fatalf("signaled updater must be outcome_unknown, err=%v",signaledErr)};exitErr:=exec.Command("sh","-c","exit 7").Run();if exitErr==nil||platformUpdateWaitOutcomeUnknown(exitErr){t.Fatalf("normal nonzero updater exit must be deterministic failure, err=%v",exitErr)};if !platformUpdateWaitOutcomeUnknown(exec.ErrNotFound){t.Fatal("unclassified wait error must fail closed to outcome_unknown")} }
