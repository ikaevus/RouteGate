...
func TestMTProtoAdapterStagesStrictConfigAndChecksBinary(t *testing.T) {
	...
	adapter.run = func(_ context.Context, name string, args ...string) ([]byte, error) {
		if name != "mtg-test" || strings.Join(args, " ") != "version" { t.Fatalf("unexpected command: %s %v", name, args) }
	...
	if err != nil { t.Fatalf("stage MTProto: %v", err) }
	if _, err := os.Stat(staged.StagedPath); err != nil { t.Fatalf("staged config missing: %v", err) }
	if _, err := adapter.Validate(context.Background(), staged.StagedPath); err != nil { t.Fatalf("validate MTProto: %v", err) }
}

func TestMTProtoParserRejectsUnknownAndUnsafeFields(t *testing.T) {
	if _, err := parseMTProtoConfig(testMTProtoConfig() + "ad-tag = \"x\"\n"); err == nil { t.Fatal("expected unknown field rejection") }
	unsafe := strings.Replace(testMTProtoConfig(), "auto-update = false", "auto-update = true", 1)
	if _, err := parseMTProtoConfig(unsafe); err == nil { t.Fatal("expected auto update rejection") }
}
...
func TestSelectVPNCoreAdapterUsesMTProtoDescriptor(t *testing.T) {
	...
	if err != nil { t.Fatalf("select MTProto: %v", err) }
	if selected.Descriptor().Protocol != platform.VPNProtocolMTProto { t.Fatalf("unexpected adapter: %+v", selected.Descriptor()) }
}
