...
func TestObservabilityFoundationPersistenceBoundaries(t *testing.T) {
	...
	rows, err := pool.Query(ctx, `
		...
		WHERE table_schema = 'public'
		  AND table_name LIKE 'observability_%'
		ORDER BY table_name
	`)
	if err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES (
			'host.disk.capacity:server-1',
			'host.disk.capacity.critical',
			'server',
			'server-1',
			'critical',
			'firing',
	...
	if _, err := pool.Exec(ctx, `
		INSERT INTO observability_alert_acknowledgements (alert_id, note)
		VALUES ($1::uuid, 'Administrator is investigating')
	`, alertID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		WHERE id = $1::uuid
	`, alertID).Scan(&conditionState); err != nil {
	...
	if _, err := pool.Exec(ctx, `
		...
		SET condition_state = 'resolved', resolved_at = now(), last_evaluated_at = now(), updated_at = now()
		WHERE id = $1::uuid
	`, alertID); err != nil {
	...
	if _, err := pool.Exec(ctx, `
		...
		VALUES (
			...
			'critical',
			'firing',
			'Disk usage critically high again',
			now(),
			now(),
			now()
		)
