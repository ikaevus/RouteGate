...
func TestHysteria2AdapterUsesLoopbackACMEBridgeOnHybridNode(t *testing.T) {
	config := buildRenderedConfig(ServerConfigInfo{
		ID: "11111111-1111-1111-1111-111111111111", Name: "hybrid", DeploymentRole: "hybrid", Status: "active",
		VPNProtocol: "hysteria2", Hysteria2Port: 443, Hysteria2Domain: "hy2.example.com",
		Hysteria2ACMEEmail: "ops@example.com", Hysteria2MasqueradeURL: "https://www.cloudflare.com/",
		VPNAccounts: []VPNAccountConfigInfo{{
			ID: "22222222-2222-2222-2222-222222222222", DisplayName: "Alice", Status: "active",
			Hysteria2Password: testHysteria2Password, TrafficEnforcementStatus: "not_enforced",
	...
	parsed, err := parseHysteria2ServerConfig(config.Hysteria2)
	if err != nil {
		t.Fatalf("parse rendered Hysteria2 config: %v", err)
	}
	if parsed.ACME.ListenHost != hysteria2ACMEBridgeHost || parsed.ACME.HTTP == nil || parsed.ACME.HTTP.AltPort != hysteria2ACMEBridgePort {
		t.Fatalf("unexpected Hybrid ACME bridge: %+v", parsed.ACME)
	}
	validation := ValidateRenderedConfig(config)
	if !validation.Valid {
		t.Fatalf("Hybrid Hysteria2 config should be valid: %+v", validation.Errors)
	}
}
