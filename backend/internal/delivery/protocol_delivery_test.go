...
func TestBuildProtocolAccessBundleCoversSupportedProtocols(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, bundle ProtocolAccessBundle) {
				if bundle.URI == "" || bundle.QRPayload != bundle.URI || bundle.ConfigText != "" { t.Fatalf("unexpected VLESS bundle: %+v", bundle) }
			},
		},
		{
			name: "shadowsocks",
			...
			assert: func(t *testing.T, bundle ProtocolAccessBundle) {
				if !strings.HasPrefix(bundle.URI, "ss://") || bundle.QRPayload != bundle.URI { t.Fatalf("unexpected Shadowsocks bundle: %+v", bundle) }
			},
		},
		{
			name: "wireguard",
			...
			assert: func(t *testing.T, bundle ProtocolAccessBundle) {
				if bundle.URI != "" || bundle.ConfigText == "" || bundle.QRPayload != bundle.ConfigText || bundle.ConfigFilename != "felix-phone.conf" { t.Fatalf("unexpected WireGuard bundle: %+v", bundle) }
			},
		},
		{
			name: "mtproto",
			...
			assert: func(t *testing.T, bundle ProtocolAccessBundle) {
				if !strings.HasPrefix(bundle.URI, "tg://proxy?") || !strings.HasPrefix(bundle.AlternativeURI, "https://t.me/proxy?") { t.Fatalf("unexpected MTProto bundle: %+v", bundle) }
			},
		},
		{
			name: "hysteria2",
			...
			assert: func(t *testing.T, bundle ProtocolAccessBundle) {
				if !strings.HasPrefix(bundle.URI, "hysteria2://") || bundle.QRPayload != bundle.URI { t.Fatalf("unexpected Hysteria2 bundle: %+v", bundle) }
			},
		},
	}
	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil { t.Fatalf("build bundle: %v", err) }
			if bundle.Protocol != test.connection.Protocol || bundle.DisplayName == "" || bundle.PrimaryAction == "" { t.Fatalf("incomplete bundle: %+v", bundle) }
...
func TestProtocolAwareTemplatesAndBrandingRenderInEnglishAndRussian(t *testing.T) {
	...
	for _, locale := range []string{"en", "ru"} {
		...
		if err != nil { t.Fatalf("render %s: %v", locale, err) }
...
func TestDefaultDeliveryBrandingCanBeDisabledExplicitly(t *testing.T) {
	...
	if text != "body" || htmlBody != "<p>body</p>" { t.Fatalf("disabled branding changed message: %q %q", text, htmlBody) }
}
