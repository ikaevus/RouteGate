...
func (h *Handler) CreateForVPNAccount(w http.ResponseWriter, r *http.Request) {
	...
	providerName, recipient, err := normalizeChannelRecipient(request.Channel, request.Recipient)
	if err != nil {
		httpx.WriteJSON(w, http.StatusBadRequest, httpx.Error(err.Code, err.Message))
		return
	}
