...
--
-- Historical hosts may report later migrations as applied while still missing
-- a physical invariant if an older copy of a migration was applied before a
-- repair was introduced. This new migration version sorts immediately after
-- 000130, so it executes on those hosts without changing the canonical latest
-- schema marker (000131).
