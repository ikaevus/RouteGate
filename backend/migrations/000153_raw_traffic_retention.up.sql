-- Preserve materialized daily traffic totals only for the explicit RouteGate
-- raw-history archival transaction. All ordinary deletes continue to subtract
-- their values from traffic_usage_daily.

CREATE INDEX IF NOT EXISTS idx_traffic_usage_events_observed_at
    ON traffic_usage_events(observed_at);
...
BEGIN
    ...
    IF current_setting('routegate.preserve_traffic_rollup', true) = 'on' THEN
        RETURN OLD;
    END IF;
