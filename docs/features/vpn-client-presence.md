...
For managed VLESS inbounds, the Agent collects presence without a separate adapter process. It reads the active sing-box configuration to map the authenticated user name to its VLESS credential UUID and correlates successful authentication records from the `sing-box.service` journal with currently established TCP sockets reported by `ss`. A live authenticated socket is reported as exact online presence. Because VLESS clients commonly open short-lived TCP flows rather than one permanent tunnel, a named successful authentication remains visible for 75 seconds as heuristic recent activity after its socket closes. The UI labels these states separately. Manager resolves the credential UUID to an active account on the Agent's own server; an unauthenticated socket or a foreign credential is never accepted.
