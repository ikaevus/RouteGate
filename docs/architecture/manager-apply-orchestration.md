...
A socket connection failure before the request is accepted is an apply-job failure with no automatic retry. A returned `ERR` is an apply-job failure. A returned `OK` is an apply-job success.
...
- it must not delete the staged candidate as part of unknown-outcome recovery;
- a later operator workflow may run fresh preflight/version checks before deciding whether a new apply attempt is safe.
...
Manager must permit at most one non-terminal local `apply` job at a time. The existing root transaction lock remains the final privileged serialization boundary and must not be replaced by Manager-side coordination.
...
- staged-artifact garbage collection policy.
