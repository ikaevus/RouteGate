...
| --- | --- | --- | --- | --- |
| Hiddify (recommended) | Standard Base64/share-link subscription | Not embedded in the primary subscription | Compatible with client-side setup | Configure routing, DNS, and TUN behavior in Hiddify; explicit `format=sing-box` is Advanced/diagnostic only |
...
| v2rayN (desktop) | Base64 standard share links | Separate native custom-rules URL: `/sub/<token>?format=v2rayn-routing` | Supported with client-side setup; manual runtime validation pending | Add the access URL under Subscription group settings, run Update subscription, then configure routing mode and verify DNS/TUN behavior |
| v2rayNG (Android) | Base64 standard share links | Not offered — routing-rules import has not been independently validated on this client | Connection only | Standard subscription import/refresh only; routing remains client-local |
| Generic | Standard VLESS/WireGuard/Shadowsocks/Hysteria2/MTProto material | Not assumed | Connection only | Configure routing in the selected client before relying on split tunneling |
...
| Full sing-box config import used by primary delivery | No | No | No | No | No |
| TUN functionality | Yes | Client feature; not managed by RouteGate | Yes | Yes | Client-dependent |
| Client-local DIRECT/VPN/BLOCK capability | Yes | **Not claimed while safety-disabled** | Yes | Not claimed | Not claimed |
| RouteGate routing-policy artifact | Not embedded in primary delivery | **Not emitted** | Native custom-rules JSON URL | Not offered | Not offered |
| Client-local DNS/split-DNS capability | Yes | Not claimed | Yes | Not claimed | Not claimed |
| Connection subscription refresh | Yes | Yes | Yes | Yes | Client-dependent |
| Routing-policy refresh/import | Client-local; not carried by primary link | N/A while disabled | Separate routing URL refresh | N/A | N/A |
| Imported RouteGate routing precedence known | Client settings govern | N/A | Client routing profile governs once selected | N/A | N/A |
...
- Hiddify uses the portable Base64 share-link subscription for VLESS, Hysteria2, and Shadowsocks and is `client_setup_required`; its normal import is not treated as a full RouteGate routing-policy import. Protocols without a share-link representation fall back to `connection_only`.
- The explicit sing-box client path remains `full_smart_routing` only for VLESS, where RouteGate can deliver the complete sing-box representation.
...
1. Hiddify connection import uses the portable standard subscription. Ozon/Wildberries DIRECT behavior must be validated against explicit client-local Hiddify routing settings, not inferred from subscription success.
...
- Hiddify current source accepts standard remote profiles, but its normal import path must not be assumed to preserve a complete provider-owned sing-box route/DNS configuration. RouteGate therefore uses the portable Base64 subscription as the primary Hiddify contract and exposes full sing-box JSON only as an explicit Advanced/diagnostic format.
